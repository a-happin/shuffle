#> shuffle:_impl/lmt/at/size/impl
#@within * shuffle:**
#@input
#  storage : _[-1].a [I; int] temporary
#  storage : _[-1].lmt List-Mapped Trie
#  score $size _shuffle (0 <= size < 65536)
...
execute store result storage : _[-1].a[0] long 131072 run scoreboard players operation $prev_index _shuffle = $size _shuffle

execute unless data storage : _[-1].lmt[0] run data modify storage : _[-1].lmt set value [[],[]]
execute if score $size _shuffle matches 0..32767 if data storage : _[-1].lmt[2] run data remove storage : _[-1].lmt[2]
execute unless score $size _shuffle matches 0..32767 unless data storage : _[-1].lmt[2] run data modify storage : _[-1].lmt append value []
